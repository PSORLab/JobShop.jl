...
function create_problem(::Subproblem, jsprob::JobShopProblem, I::Vector{Int}, λ)
    @unpack J, Tp, T, R, p, ps, pr, w, d, U, Om, Mi, M, y, τ, sslackk, stard1, stard2, ta, tb, tc = jsprob
    ...
    @expression(model, tard2[i = I, j = J[i], r = 1:2], W2[3,i,j,r]*2*Tp[end])
    @expression(model, total_tard, sum(ta[i]*tard1[i] for i = I) +
                                   ...
                                   sum(λ[m,t]*sslackk[m,t] + penalty*v_p[m,t] for m = Mi, t = Tp)
                )

    @objective(model, Min, total_tard)
   
    return model, s, b1, b2
end

function create_solve!(::Subproblem, d::JobShopProblem, I, λ)
    m, s, b1, b2 = create_problem(Subproblem(), d, I, λ)
    optimize!(m)
