...
function display_iteration(j::JobShopProblem, i::Int)
    @unpack maxest, estimate, penalty, current_iteration, time_start, time_subprob, 
    ...
    if (current_upper_bound(j) < 100000.0) || (j.parameter.verbosity > 1)
        ...
        outstring *= @sprintf("Total Time: %.2e  %.2e  %.2e ", total_time, time_subprob, time_solve_stepsize)
